...
func TestHandleRPush(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := store.NewStore()
			if len(tt.stored) > 0 {
				_, err := commands.HandleRPush(s, tt.stored)
				if err != nil {
					t.Fatalf("Unexpected error %v", err)
				}
			}

			r, err := commands.HandleRPush(s, tt.input)
			if err != nil {
				t.Fatalf("Unexpected error %v", err)
			}
...
func TestHandleInvalidRPush(t *testing.T) {
	tests := []struct {
		name          string
		keyValuePairs []KV
		input         []resp.CashewValue
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := commands.HandleRPush(s, tt.input)
			if err == nil {
