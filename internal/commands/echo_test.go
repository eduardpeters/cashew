...
func TestHandleEcho(t *testing.T) {
	tests := []struct {
		...
		{"Replies with first argument as bulk string",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r, err := commands.HandleEcho(tt.input)
			if err != nil {
...
func TestHandleEchoInvalidArguments(t *testing.T) {
	tests := []struct {
		name  string
		input []resp.CashewValue
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := commands.HandleEcho(tt.input)
			if err == nil {
