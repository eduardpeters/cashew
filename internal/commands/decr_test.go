...
func TestHandleDecr(t *testing.T) {
	tests := []struct {
		...
		{"replies with 0 if key is set with value 1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r, err := commands.HandleDecr(s, tt.input)
			if err != nil {
...
func TestHandleInvalidDecr(t *testing.T) {
	tests := []struct {
		name          string
		keyValuePairs []KV
		input         []resp.CashewValue
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := commands.HandleDecr(s, tt.input)
			if err == nil {
